...
SELF_BANNED = 'Estas baneado: {reason}'
PLAYER_BANNED = '{name} has sido baneado: {reason}'
...
def banip(script, ip, *reason):
    ...
    return f'{len(banned)} jugadores baneados'
...
def unban(script, ip):
    ...
    if script.parent.unban(ip):
        return 'IP "%s" desbaneado' % ip
    else:
        return 'IP no encontrado'
