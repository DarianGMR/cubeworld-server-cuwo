...
class IRCScriptConnection(ConnectionScript):
    def on_join(self, event):
        self.parent.send('* %s entro al juego' % self.connection.name)

    def on_unload(self):
        if not self.connection.has_joined:
            return
        self.parent.send('* %s desconectado' % self.connection.name)
...
def who(bot):
    ...
    if player_count == 0:
        bot.me('No tiene jugadores conectados')
        return
    formatted_names = []
    ...
    msg = 'tiene %s %s conectado: ' % (player_count, noun)
