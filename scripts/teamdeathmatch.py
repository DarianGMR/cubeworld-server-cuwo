"""
Script de Duelo por Equipos de Sarcen

Características:
Dos equipos
Rachas de asesinatos
Asesinatos multiples
Experiencia por asesinato y asistencia

No debe usarse junto con el script de PvP, a menos que quieras que los miembros del equipo
puedan matarse entre sí.
"""
...
class TDMConnection(TeamConnection):
    ...
    spree_kill_count = 3
    spree_names = ['%s esta en una ola de asesinatos!',
                   '%s esta descontrolado!',
                   '%s esta dominando!',
                   '%s es imparable!',
                   '%s es muy bueno!']
    ...
    def on_player_kill(self, player):
        ...
        message += '%s asesino a %s' % (killer.name, killed.name)
        ...
        if killed_spree > 0:
            ...
            message += ', poniendo fin a la ola de asesinatos de %s.' % his_her
...
class TDMServer(TeamServer):
    ...
    def get_mode(self, event):
        return 'Combate a muerte por equipos'

    def on_load(self):
        super(TDMServer, self).on_load()

        self.create_team('Rojo')
        self.create_team('Azul')
    ...
    def declare_winner(self, team):
        if self.round_active:
            ...
            message = 'El equipo "%s" ha alcanzado %s bajas y ha ganado la ronda!'
    ...
    def set_max_score(self, score):
        ...
        message = 'Puntuacion maxima de combate por equipos establecido en %s' % score
        self.server.send_chat(message)
        return message
    ...
    def start_round(self):
        ...
        message = '¡Combate a muerte por equipos! El primer equipo en alcanzar %s bajas gana..' % \
                  self.max_score
        print(message)
        self.server.send_chat(message)
    ...
    def start_round_delayed(self):
        ...
        self.announcer.action = 'Inicio de ronda'
        self.announcer.abort_message = 'ronda abortada.'
        self.announcer.message = "{time}"
        self.announcer.message_long = "La ronda comienza en {time} segundos."
