"""
Script de equipos de Sarcen

Este script crea equipos de jugadores para PvE y PvP.

No debe usarse junto con el script de PvP, a menos que quieras que los miembros del equipo puedan matarse entre sí.

Este script está diseñado como una biblioteca que debe importarse a otros scripts.
Ver teamdeatmatch.py como ejemplo.
"""
...
class TeamConnection(ConnectionScript):
    ...
    def on_player_kill(self, player):
        ...
        self.server.send_chat('%s asesino a %s!' % (killer,
                                                 killed))
    ...
    def invite_team(self, t):
        if t not in self.team_invites:
            ...
            message = 'Has sido invitado a unirte al equipo "%s".' % t.name
...
class Team(object):
    ...
    def send_chat(self, message, exception=None, source=None):
        prefix = "[LIDER] " if source == self.leader else "[TEAM] "
    ...
    def add(self, script, is_invited):
        ...
        if self.invite_only and not is_invited:
            message = 'El equipo "%s" es solo por invitacion..' % self.name
            script.connection.send_chat(message)
            return False

        self.members.append(script)
        script.connection.send_chat('Te has unido al equipo "%s".' % self.name)
        message = '%s se ha unido a tu equipo.' % script.connection.name
    ...
    def set_invite_only(self, invite_only):
        ...
        message = 'solo invitar' if self.invite_only else 'abierto'
        return 'El equipo esta ahora %s.' % message
    ...
    def set_leader(self, leader):
        self.leader = leader
        message = 'Ahora eres el lider del equipo de "%s".' % self.name
        self.leader.connection.send_chat(message)

        message = '%s ahora es el lider del equipo de "%s".' % (leader.connection.name,
                                                      self.name)
    ...
    def remove(self, script):
        ...
        script.connection.send_chat('has dejado el equipo "%s".' % self.name)
        message = '%s ha dejado tu equipo.' % script.connection.name
        self.send_chat(message, script)
...
class TeamServer(ServerScript):
    ...
    def team_list(self, script):
        message = '%s Equipo(s): ' % len(self.teams)
        ...
        if len(teams_closed) > 0:
            message += ' Solo por invitación: ' + ', '.join(teams_closed)
        if len(teams_open) > 0:
            message += ' Abierto: ' + ', '.join(teams_open)

        return message

    def team_create(self, script, name):
        if self.locked_teams:
            return 'Los equipos estan bloqueados.'

        if name in self.teams:
            message = 'Ya existe un equipo con el nombre "%s".' % name
            return message
    ...
    def team_join(self, script, name):
        ...
        if t is None:
            return 'No hay ningun equipo llamado "%s"' % name
