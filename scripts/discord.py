...
class DiscordClient(discord.Client):
    ...
    async def on_ready(self):
        print(f'Inicio sesion en Discord como {self.user.name} {self.user.id}')
    ...
    async def on_server_join(self, server):
        ...
        print('Inicia sesion en el canal de Discord especificado.')
...
class DiscordScriptConnection(ConnectionScript):
    def on_join(self, event):
        self.parent.send(f'{self.connection.name} entro al juego')

    def on_unload(self):
        if not self.connection.has_joined:
            return
        self.parent.send(f'{self.connection.name} desconectado')
